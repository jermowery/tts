...
public class ApiController {
    private static final Logger logger =
            Logger.getLogger(ApiController.class.getName());
    ...
    public SseEmitter getAudioFile(@RequestBody ConvertRequest request) {
        ...
        responseHeaders.set("Content-Disposition",
                "attachment; filename=\"combined.mp3\"");

        SseEmitter emitter = new SseEmitter();

        executor.submit(() -> {
            try (TextToSpeechClient textToSpeechClient = TextToSpeechClient.create()) {
                ImmutableList<Future<Void>> chunks = request.getParts()
                        .stream()
                        .flatMap(part -> {
                            ...
                            try {
                                return ssmlProvider.getBlocks(new StringReader(part.getText())).stream()
                                        .map(chunk -> executor
                                                .submit(() -> emitAudio(textToSpeechClient, chunk, part.getVoice(), emitter)));
                            } catch (Exception e) {
    ...
    private Void emitAudio(TextToSpeechClient client, String text,
                           String requestedVoice, SseEmitter emitter) {
        ...
        ApiFuture<SynthesizeSpeechResponse> response = client.synthesizeSpeechCallable()
                .futureCall(request);
        try {
